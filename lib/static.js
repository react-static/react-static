          ...
          case 7:
            ...
            return _context2.abrupt('return', Promise.all(config.routes.map(function () {
              var _ref3 = _asyncToGenerator( /*#__PURE__*/_regenerator2.default.mark(function _callee(route) {
                var initialProps, URL, InitialPropsContext, ContextualComp, routesList, renderMeta, appHtml, helmet, head, HtmlWithMeta, HeadWithMeta, BodyWithMeta, html, htmlFilename, initialPropsFilename, writeHTML, writeJSON;
                  ...
                  while (1) {
                    switch (_context.prev = _context.next) {
                      ...
                      case 9:
                        ...
                        _context.next = 17;
                        return config.renderToHtml(_server.renderToString, ContextualComp, renderMeta);
        ...
        switch (_context4.prev = _context4.next) {
          case 0:
            ...
            templateImports = templates.map(function (template) {
              return 'import ' + template.replace(/[^a-zA-Z]/g, '_') + ' from \'' + _path2.default.resolve(config.paths.ROOT, template) + '\'';
            }).join('\n');
            templateMap = 'const templateMap = {\n    ' + templates.map(function (template, index) {
              return 't_' + index + ': ' + template.replace(/[^a-zA-Z]/g, '_');
            }).join(',\n') + '\n  }';
            tree = {};
