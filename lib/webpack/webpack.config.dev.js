...
var _default = function _default(_ref) {
  ...
  var hotPatches = config.noHot ? [] : [require.resolve('react-hot-loader/patch'), require.resolve('react-dev-utils/webpackHotDevClient'), require.resolve('webpack/hot/only-dev-server')];
  return {
    context: _path2.default.resolve(__dirname, '../node_modules'),
    entry: [].concat(hotPatches, [
    // `${require.resolve('webpack-dev-server/client')}?/`,
    _path2.default.resolve(ROOT, config.entry)]).filter(function (v) {
      return v;
    }),
    output: {
    ...
    }), !config.noHot && new _webpack2.default.HotModuleReplacementPlugin(), new _webpack2.default.NoEmitOnErrorsPlugin(), new _webpack2.default.NamedModulesPlugin(), new _caseSensitivePathsWebpackPlugin2.default()].filter(function (v) {
      return v;
    }),
